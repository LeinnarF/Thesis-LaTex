...
Algorithm perforamcne was evaluated using two primary metrics derived from the averged iterations:
...
\end{equation}
Where \( r_{ij} \) is the normalized value of the \( j \)-th criteria for the \( i \)-th algorithm, \( x_{ij} \) is the original value, and \( \max{x_{ij}} \) is the maximum value observed for that criteria.
...
For the parameters of AGAM, the following values were used: $pc_1 = 0.9$, $pc_2 = 0.6$, $pm_1 = 0.1$, and $pm_2 = 0.01$. These values were chosen based on \textcite{Yochum2020} configurations. 
