...
\textbf{Algorithm Ranking.} To select the final algorithm, the trade-off between speed and accuracy was resolved using Simple Additive Weighting (SAW). Since both $T _{avg}$ and $Q_{avg}$ are “cost” criteria, i.e., lower values represent better performance, it is normalized relative to the maximum (worst) observed value to scale all metrics between 0 and 1. The normalization formula used was shown in equation \ref{eq:norm}:
\begin{equation}
	\label{eq:norm}
...
For this study, since the weights can be subjectively assigned, 
