...
\begin{table}[H]
	...
	\label{tsplib_instace} \renewcommand{\arraystretch}{0.6}
	\begin{tabularx}{\linewidth}{lX}
		\toprule
		\textbf{TSPLIB instance} & \textbf{Description} \\
		\midrule
		burma14 & Small instance with 14 cities. Optimal solution known: 3323. \\ 
		bays29 & Medium instance with 29 cities. Optimal solution known: 2020. \\ 
		eil51 & Large instance with 51 cities. Optimal solution known: 426. \\ 
		berlin52 & Large instance with 52 cities. Optimal solution known: 7542. \\
		\bottomrule
	\end{tabularx}
\end{table}

\textbf{Experimental Setup.} The experiments were implemented in Python because of its library support for TSPLIB, and for simplicity of the language. To ensure statistical reliability and mitigate the randomness nature of heuristic algorithms, each algorithm was executed for 10 independent iterations on each TSPLIB instance. The results were averaged to get the baseline performance profile. 

\textbf{Performance Metrics.} Two primary metrics were defined to evaluate the algorihms:

\begin{enumerate}
	\item \textbf{Runtime Speed (\( T_{avg} \)).} It measures the average time it takes to find the solution accross the 10 iterations, measured in seconds. This metric is crucial for assessing the efficiency of the algorithm, and is calculated using the formula shown in equation \ref{eq:runtime}: 
	\begin{equation}
		\label{eq:runtime}
	...
	\item \textbf{Solution Quality (\( Q_{avg} \)).} Since the best solution for each TSPLIB instacs are known, the accuracy is measured by the average relative error of the solution found by the algorithm compared to the optimal solution. The calculation is shown in equation \ref{eq:quality}:
	\begin{equation}
		\label{eq:quality}
...
\textbf{Algorithm Ranking.} To select the final algorithm, the trade-off between speed and accuracy was resolved using simple additive weighting (SAW). Since both Runtime speed and Average relative error are “cost” criteria, i.e., lower values represent better performance, it was normalized relative to the maximum (worst) observed value to scale all metrics between 0 and 1. The normalization formula used was:
\begin{equation}
	\label{eq:norm}
...
The final score $S _{i}$ for each algorithm $i$ was calulated in equation \ref{eq:saw}: 
\begin{equation}
	\label{eq:saw}
...
All algorithm will be executed on the same hardware and software environment to ensure consistency in the results. Since the algorithms are metaheuristic, they will be run multiple times on each dataset to account for variability in performance. The average results will be used for comparison. 

\noindent For every run, the following metrics will be recorded:
\begin{itemize}
	\item The average solution quality (total travel distance).
	\item The best solution found in each run.
	\item The average computation time taken to reach the solution.
\end{itemize}
The algorithm that performs best in terms of solution quality and runtime efficiency will be selected for implementation in the itinerary optimization feature of the mobile application.
