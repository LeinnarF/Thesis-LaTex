...
PM =
\begin{cases}
pm_1 - \dfrac{(pm_1 - pm_2)(f _{max} - f)}{f_{max} - f_{avg}}, & f \ge f_{avg} \\
pm_1, & f < f_{avg}
\end{cases}
\]
...
\textbf{Instruments.} The study will use the following instruments: a demographic sheet capturing age, gender (optional), travel frequency, familiarity with Bulacan, and device specifications; a TAM questionnaire consisting of 10-point Likert items (1 = strongly disagree, 10 = strongly agree) covering Perceived Usefulness, Perceived Ease of Use, Attitude Toward Use, and Intent to Use (3–5 items per construct); an ISO/IEC 25010 checklist for professionals with 10-point ratings per characteristic (Functional Suitability, Performance Efficiency, Compatibility, Usability/Interaction Capability, Reliability, Security, Maintainability, Flexibility) plus open-ended comments; and a task script in which users perform core flows (generate a personalized itinerary, optimize an itinerary, manage items, and start navigation) before answering the TAM.

\textbf{Procedure.} Data collection will begin with orientation and informed consent, after which an anonymous respondent code will be assigned. The app will then be installed on the participant’s device and a brief training on basic controls will be provided. Participants will execute the task script while non-identifying usage metrics (e.g., task completion and errors) may be logged. End users will complete the TAM, while professionals will complete the ISO/IEC 25010 instrument and provide qualitative feedback. Responses will be submitted via a secured online form and exported to CSV for analysis. No personally identifying data will be collected other than optional contact for follow-ups, and all data will be stored on encrypted drives accessible only to the research team.

\subsection{Data Processing and Analysis}
% Includes:
%   Descriptive statistics
%   10 point scale (see sample )
%   Mean and SD for each category
%   Verbal interpretation

The collected data will be interpreted in a 10-point scale, aligning with ISO/IEC 25010 standards and TAM criteria. These standards will evaluate system quality and user acceptance across multiple dimensions. Where 1 is equivalent to completely disagree, and 10 is equivalent to completely agree. 

\begin{figure}[H]
    \centering
    \caption{10-point Scale}
    \label{pointScale}
    \includegraphics[width=0.8\linewidth]{PointScale.png}
\end{figure}

Descriptive statistics will be utilized to summarize and interpret the collected data. For each evaluation category (e.g., TAM constructs, ISO/IEC 25010 criteria), the mean and standard deviation will be calculated to provide a quantitative overview of user and expert responses. These results will be presented in tables and figures, accompanied by verbal interpretations based on the defined scale. Open-ended responses will be analyzed thematically to highlight key insights and suggestions for system improvement.

For each category, the mean score ($\bar{x}$) and standard deviation ($s$) will be computed as follows:
\[
...
\begin{table}[H]
	\centering
	\caption{Interpretation of Acceptability Ratings}
	\label{verbalInterpretation} \renewcommand{\arraystretch}{1.0}
	\begin{tabularx}{\linewidth}{llX}
		\toprule
		\textbf{Range} & \textbf{Descriptive Rating} & \textbf{Interpretation}\\
		\midrule
...
Table \ref{verbalInterpretation} shows the interpretation of computed mean scores for system acceptability and quality. This mapping provides a clear verbal description for each range, helping readers understand the practical implications of the quantitative results. 
