...
The applied research aspect of the study will focus on developing and implementing a system for generating personalized itineraries and optimizing existing itineraries for the interests of the user. The primary objective of this study is to create a mobile application that optimizes itineraries using the best Metaheuristic algorithm and generates personalized itineraries with travel budget using Adaptive Genetic Algorithm with Dynamic Mutation and Crossover Probabilities (AGAM) algorithm. Through an applied research approach, the features to be implemented in the system will be important for the needs of its intended users which are the tourists and their itinerary needs.
...
\subsection{Modeling Route Optimization using TSP}
Route optimization refers to the process of finding the optimal order in visiting a set of places. This is also another definition of the TSP, where the salesman must minimize the travel cost between traveling a set of cities. In the study, the POI will be represented as nodes and the edges connecting each node as the distance between the shortest route given by the Mapbox API where a sample graph is shown in Figure \ref{tspgraph}. 
...
\end{equation}
where $n$ is the total number of vertices, $c_{ij}$ refers to the cost of traveling from vertex $i$ to vertex $j$ and $x_{ij}$ refers to a binary value if an edge directly connecting vertex $i$ to $j$ exists. The function is also constrained to the following:
\begin{enumerate}
...
Using the exact approaches such as bruteforce or integer programming, the TSP can be calculated with utmost confidence as the most optimized result. However, this is only feasible for small graphs with small vertices as the computation cost scales with the factorial. But the user can’t be expected to wait hours or days for an optimization of a 10-day itinerary so the only option is through heuristic algorithms. 

The expected runtime for the algorithm should be fast but also accurate. There are many optimization algorithms that have been used in the field of TSP optimization. However, due to the constraints of the processing power of a mobile device, the lightweight metaheuristic algorithms will be tested and analyzed for the algorithm’s runtime, specifically the algorithms:
\begin{enumerate}
    ...
    \item \textbf{Simulated Annealing (SA).} The Simulated Annealing (SA) algorithm is a metaheuristic technique influenced by the annealing process in metallurgy. It begins with an initial solution and incorporates a mechanism that allows for the occasional acceptance of a "worse" solution based on a probability distribution. This stochastic element helps the algorithm escape local optima and more thoroughly explore the solution space. The probability of accepting a worse solution is gradually reduced over time, simulating the cooling process. SA is applicable to a wide range of optimization problems. Algorithm \ref{SA} illustrates the stepwise SA algorithm for solving TSP \parencite{Hossain2024}.
    ...
    % \item \textbf{Hovering Scouts and Foraging Flocks Pied Kingfisher Optimizer (HSFFPKO).}
\end{enumerate}
...
\For{$t = 1$ to max\_iterations}
    \For{each clan}
        ...
        \EndFor
        \State Update clan’s best ($gBest$ candidate)
    \EndFor
    \State
...
Each algorithm will be given 10 iterations each for solving 20 TSP instances from the TSPLIB. The average runtime and accuracy will be recorded. Afterwards, the whole process can be visualized in Figure \ref{algoprocess}. There are three phases, the recording of runtime for each algorithm, performing Friedman test, and performing Nemenyi test if allowed by the Friedman test. The recording and analysis will be performed through the Python language and its respective packages.

\begin{figure}
    \centering
    \caption{Algorithm Runtime Analysis Process}
    \label{algoprocess}
    \includegraphics[width=0.5\textwidth]{friedman.jpeg}
\end{figure}

After recording the average runtime for each problem, the Friedman test will be conducted to determine if each algorithm’s runtime results differ from one another. The Friedman test is ideal for this experiment where the distribution of runtime results across diverse TSPLIB instances may not strictly follow a normal distribution.

The steps involve ranking the algorithms for each TSP instance independently. The algorithm with the fastest runtime is assigned a rank of $1$, the second fastest a rank of $2$, and so on. In cases of ties, average ranks are assigned. The test then evaluates the Null Hypothesis ($H_0$), which states that all algorithms are statistically equivalent and any differences in average runtime are only present due to the randomness in the algorithms’ implementations.

The formulation for the Friedman test is shown in Equation \ref{Friedman}, where $n$ is the number of subjects, $k$ is the number of treatments and, $\sum{R^2}$ is the squared sum of the total ranks.

\begin{equation}
    \label{Friedman}
    FM = \frac{12}{nk ( k + 1 )} \sum{R^2} - 3n ( k + 1 )
\end{equation}
where $n$ is number of the subjects, $k$ is number of treatments, and $\sum{R^2}$ is the squared sum of the total ranks.

If the calculated p-value for the Friedman test is less than the significance level $\alpha$, the null hypothesis will be rejected. Rejection of the null hypothesis will confirm that there is a statistical significant difference in the runtime of at least two algorithms. Unfortunately, the Friedman test does not identify which among the algorithms differ. Therefore, a post-hoc analysis is required.

Only if the null hypothesis is rejected, only then will the Nemenyi post-hoc test will be applied to perform pairwise comparisons. This test uses the Critical Difference (CD) value to determine if the runtime gap between any two specific algorithms is significant. If the difference in the average ranks of two algorithms exceeds the CD, their performance is considered significantly distinct.

The formula for the Nemenyi test is shown in Equation \ref{critD} where $q_\alpha$: The critical value based on the Studentized range statistic (specifically for infinite degrees of freedom). You look this up in a standard Nemenyi critical value table (e.g., for 20$\alpha=0.05$).21$k$: Number of algorithms.$N$: Number of datasets.
...
\end{equation}
where $Z$ is the total travel distance; $S_i$ is the total score of a vertex at index i such as the travel distance, user preferability score, and popularity rating; $x_i$ is the decision variable whether the vertex $i$ to vertex $j$ is included in the output. The function is also constrained to the following:
\begin{enumerate}
...
\textbf{Study Adoption. }AGAM works by taking specific factors that can affect the results of a solution. In this paper, the goal is to generate an optimal solution of a recommended itinerary, which lets the model take into account factors similar to the ones proposed by \textcite{Yochum2020}. The factor referring to mandatory POIs will from hereon refer to the interest of the user. In addition to these factors, AGAM also requires a certain probability that determines the chances of mutation and crossover to happen.
AGAM considers the population set $P = {p_1,\dots, p_n}$ where each individual $p_i = {q_1, q_2, q_3,\dots, q_n}$ is a sequence where each $q_i$ is a POI. This means that AGAM treats each POI sequence $p_i$ as an individual gene, hence $P$ as a sequence of itineraries. 

\textbf{Normalization Procedure. }\textcite{Yochum2020} noted that measuring different factors involves variables with inconsistent units and scales, and that each gene sequence may vary in length depending on the number of POIs selected. They addressed this by applying a normalization method that converts all variable values into a uniform range between 0 and 1, so that the algorithm can fairly compare and combine the factors within a fitness function.

\textbf{Fitness Function. }This fitness function normalizes the evaluation of the quality of each $p_i$ by integrating the input factors defined earlier. In this way, every itinerary $p_i$ in $P$ obtains different weights given by Equation \ref{agamFitness}.
\begin{equation}
    \label{agamFitness}
    f(p_i) = w_{1}Ti(p_i) + w_{2}Tn(p_i) + w_{3}Tr(p_i) + w_{4}Tp(p_i)
\end{equation}
Where in Equation \ref{agamFitness}, $f(p_i)$ is the fitness function score of each itinerary $p_i$. 
$w_{j}$ is the weight of each factor.
$Ti(p_i)$ is the sum of all interest values of each POI in the itinerary $p_i$. Each POI contains an interest value which quantifies how interested the user is in that specific POI category. 

In order to determine the level of interests of the user in a specific POI, the cosine similarity will be used. According to Singh et al. (2023), cosine similarity measures the angle of cosine between two vectors. This essentially provides a number for how two vectors relate to each other.  The formula for cosine similarity is in eq \ref{cosim} where $v$, $w$ are vectors and the output of the function ranges from -1 being not related to 1 essentially the same vectors. 

\begin{equation}
    \label{cosim}
    \text{cosine\_similarity}(v, w) = \frac{v \cdot w}{\|v\| \|w\|} = \frac{\sum_{i=1}^{n} v_i w_i}{\sqrt{\sum_{i=1}^{n} v_i^2} \sqrt{\sum_{i=1}^{n} w_i^2}}
\end{equation}
In the context of the application, each POI has categories which are appropriately assigned by the researchers. Then, a category matrix (\ref{matrix}) will be generated for each POI for each available category where 1 will mean POI is in that category and 0 as not.
...
\begin{equation}
    \label{matrix}
        ...
        \end{bmatrix}
\end{equation}
The column of the matrix represents the POI and the row represents the category. 

Another vector (\ref{uservector}) that represents the user’s preferences in the categories will be created. This is a binary vector where 1 represents that the user wants to see this and 0 as not wanting to see this type of category. For instance, if the user has chosen all categories to be in their preferences, in which case, all the values are one.
...
Then, the cosine similarity of each POI category matrix to the user preference will be calculated. This value from the cosine similarity now represents the user’s interest to that POI based on the preference matrix. 

$Tn(p_i)$ will be obtained by dividing the number of POIs in $p_i$, by the total number of possible POIs denoted by NOP. Also note that the selected number of POIs in $p_i$ will be limited by a user-defined parameter which will be referred to as $MAXN$, an integer limiting the number of POIs in a $N_op(p_i)$.	
$N_{op}(p_i) \leq MAXN$.
\begin{equation}
    \label{agam1}
    \dfrac{\sum_{j=1}^{n} (1)}{NOP}
\end{equation}
This ensures that AGAM does not prioritize the entirety of all of Bulacan’s POIs considered in the study.

$Tr(p_i)$ is the total rating of $p_i$ given by Equation \ref{agam2}, taking the sum of all $q_i$ rating $R(q_i)$ Equation \ref{agam4} in $p_i$ and dividing it by the best possible rating of all POIs in consideration combined $MAXR = \text{Number of POIs} * \text{Maximum Rating}$.
...
Algorithm \ref{alg:agam} is a modified pseudocode of AGAM, adjusted to fit the changes mentioned above. In this way, the utilized algorithm is better suited to cater given data, obtained from sources available to the researchers.
...
\State \Return $p_{opt}$ = itinerary with highest fitness in $P$
...
\end{enumerate}
Furthermore, a questionnaire will be provided to be answered by the user when they first open the app. The questionnaire requires the user to select what categories of POI they prefer. This approach will address the “cold start” problem of the system which ensures that AGAM will have the necessary input like user interest, which cannot be obtained from outsourced-data.
...
This framework will allow the researchers to develop the mobile application, LAKAD, as React Native’s cross-platform capabilities also allow for android development.
