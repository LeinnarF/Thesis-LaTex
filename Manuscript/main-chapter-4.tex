...
The SAW scores in Table \ref{tab:rank} showed that Simulated Annealing (SA) functioned as the best algorithm because it achieved optimal solution with efficient runtime performance. GA achieved the highest accuracy but its increasing runtime with larger problem sizes resulted in a consistently high SAW score which positioned it in 6th place. EHO performed effectively on small instances but its accuracy declined when handling larger cases which made it unsuitable for complex itineraries. This result aligns with the argument of \textcite{Wadi2025} that heuristic models, specifically the ones that balance both efficient execution time with adequate solution quality, are much more effective for environments that are resource-constrained such as in mobile. Compare this to exact methods that although prioritizes the best quality of solution, such of their highly complex algorithms risk critical resource exhaustion which is not a desirable outcome, especially in real-time optimization. Here, SA ensures that users does receive highly optimized travel sequences within a responsive, real-time time frame.
...
These acceptability scores support the claims of \textcite{Yochum2020} that AGAM's framework for multi-objective optimization problem can provide users with a richer and unique experience, a goal that is aimed to be achieved by DOT's tourism development plan \parencite{DOT2023}.
