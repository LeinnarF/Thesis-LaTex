...
\begin{enumerate}
	...
	\item The system relies on Supabase as its primary database and OSRM and Mapbox for API, which are both external. The researchers proposes using a self-hosted server to achieve better scalability and system control while delivering dependable route calculations that operates independently of external services.
\end{enumerate}
